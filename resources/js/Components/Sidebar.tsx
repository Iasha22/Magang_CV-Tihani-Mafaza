...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
    ...
    return (
        <>
            ...
            <aside
                className={`fixed top-0 left-0 z-50 h-screen w-72 flex flex-col justify-between border-r border-slate-800 bg-slate-950/95 backdrop-blur-2xl transition-transform duration-300 lg:translate-x-0 ${
            ...
            >
                ...
                <div>
                    <div className="flex items-center gap-3 px-6 py-6 border-b border-slate-800/80">
                        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-tr from-indigo-600 to-indigo-400 text-white shadow-lg shadow-indigo-500/25">
                            <Building2 className="h-6 w-6" />
                        </div>
                        <div>
                            <div className="flex items-center gap-2">
                                <span className="text-base font-bold text-white tracking-wide">CV TIHANI</span>
                            ...
                            <p className="text-xs text-slate-400">Sistem Informasi Akuntansi</p>
                        </div>
                    </div>

                    {/* Navigation */}
                    <div className="px-4 py-4 space-y-6 overflow-y-auto max-h-[calc(100vh-190px)]">
                        {navItems.map((group) => (
                            <div key={group.group}>
                ...
                <div className="p-4 border-t border-slate-800/80 bg-slate-900/40">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-800 text-indigo-400 font-bold border border-slate-700">
                                {user?.name ? user.name.charAt(0).toUpperCase() : 'I'}
                            </div>
                            <div className="overflow-hidden">
                                <p className="text-sm font-semibold text-white truncate">{user?.name || 'Iasha Tsamrotul F.'}</p>
                                <div className="flex items-center gap-1.5 text-[11px] text-slate-400">
                                    <span className="inline-block h-1.5 w-1.5 rounded-full bg-emerald-400"></span>
                                    <span className="capitalize">{user?.role || 'Admin Akuntansi'}</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="mt-3 pt-2 border-t border-slate-800/60 text-[10px] text-slate-400 flex items-center justify-between">
                        <span>KP & TA STMIK Mardira</span>
                        <span className="font-mono">v1.0-KP</span>
                    </div>
                </div>
            </aside>
        </>
